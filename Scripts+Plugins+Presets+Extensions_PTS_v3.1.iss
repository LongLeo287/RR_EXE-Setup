// --------------------------------- INFOMATION ------------------------------- //
//                                                                              //
//               EXE For File Srcipts/Plugins/Presets/Extensions PTS            //
//               Script Version:	v3.1                                          //
...
begin
  ...
  begin
    TempADir := ExpandConstant('{tmp}' + '\Actions');
    TempBDir := ExpandConstant('{tmp}' + '\Brushes');
    TempCDir := ExpandConstant('{tmp}' + '\Extensions');
   ...
   ////////////////////////////////[ Folder ADOBE\Scripts + Plugins + Presets ]/////////////////////////////////
    ...
    //1. File .atn C:\Program Files\Adobe\Adobe Photoshop [xxxx]\Presets\Actions | 2. File .abr C:\Program Files\Adobe\Adobe Photoshop [xxxx]\Presets\Brushes
    #define outputFolder GetEnv("ProgramW6432") + "\Adobe"
    #define outputA "\Presets\Actions"
    #define outputB "\Presets\Brushes"
    #define appFolder "Adobe Photoshop"
      Paths := GetDirectories('{#outputFolder}', '{#appFolder}');
      try
        for i := 0 to Paths.Count - 1 do
        begin
          // Copy all files and folders from SourceDir to each destination in Paths
          CopyFilesAndFolders(TempADir, Paths[i] + '{#outputA}', ConfigFileContent);
          CopyFilesAndFolders(TempBDir, Paths[i] + '{#outputB}', ConfigFileContent);
        end;
      finally
        Paths.Free;
      end;
  //C:\Program Files (x86)\Common Files\Adobe\CEP\extensions
    #define outputAnother GetEnv("ProgramFiles") + "\Common Files\Adobe\CEP\extensions\" 
      if not DirExists('{#outputAnother}') then ForceDirectories('{#outputAnother}');
      CopyFilesAndFolders(TempCDir, '{#outputAnother}', ConfigFileContent); 
